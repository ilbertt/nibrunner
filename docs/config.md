...
| `[proxy.passthrough]` | `max_ports_per_app` | how many ports may reach an app unread |
...
**`[proxy.passthrough]` is the way in for a protocol this host does not read** — ssh, in practice. Such a
...
host with no `[proxy.http]`, or more ports than `[proxy.passthrough]` allows.
