...
`max_apps` is the first key in `config.toml`, above every section:
...
reserves from 21000, the nbd minors on a `zerofs` host, and what the metrics page calls
`nibrunner_slots{of="total"}`. A host that is full says so by naming this key.
...
<Cards>
  <Card title="Sleep and wake" href="/docs/guides/sleep-and-wake" description="How a host holds more apps than it runs at once." />
  <Card title="config.toml" href="/docs/reference/config" description="max_apps, key by key with the rest." />
</Cards>
