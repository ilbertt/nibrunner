...
impl RunningHost {
    ...
    /// Built through the fixture beside the protocol's own types, so that a field added to the
    /// document is not a field this harness has to be told about.
    pub fn document(&self, apps: &[Tenant]) -> HostDesiredState {
        super::desired_state(|document| {
            document.volumes = apps.iter().map(|app| app.volume.clone()).collect();
            document.instances = apps.iter().map(|app| app.instance.clone()).collect();
        })
    }
