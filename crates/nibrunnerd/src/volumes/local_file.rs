...
impl LocalFileVolumes {
    pub fn new(directory: PathBuf, storage_prefix: ObjectKey, commands: Arc<dyn CommandRunner>) -> Self {
        Self { directory, storage_prefix, commands }
    }
    ...
    fn size_of(path: &Path) -> Option<u64> {
        std::fs::metadata(path).ok().filter(|info| info.is_file()).map(|info| info.len())
    }
    ...
    fn ensure_file(&self, volume_id: &VolumeId, size_bytes: u64) -> Result<u64, VolumeError> {
        ...
        if let Some(current) = Self::size_of(&path) {
            if current > target {
                return Err(VolumeError::ShrinkRefused { current, requested: target });
            }
            if current == target {
                return Ok(target);
            }
        }
        make_directory(&self.directory, VOLUME_DIR_MODE)
            .map_err(|error| VolumeError::Unusable(format!("{} could not be made: {error}", self.directory.display())))?;
        let file = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(&path)
            .map_err(|error| VolumeError::Unusable(format!("{} could not be opened: {error}", path.display())))?;
        file.set_len(target)
            .map_err(|error| VolumeError::Unusable(format!("{} could not be sized: {error}", path.display())))?;
        #[cfg(unix)]
        {
    ...
    fn is_formatted(&self, volume_id: &VolumeId) -> Result<bool, VolumeError> {
        ...
        let mut file = std::fs::File::open(&path)
            .map_err(|_| VolumeError::SuperblockUnreadable { device_path: path.display().to_string() })?;
        if file.seek(SeekFrom::Start(SUPERBLOCK_MAGIC_OFFSET)).is_err() {
            return Err(VolumeError::SuperblockUnreadable { device_path: path.display().to_string() });
        }
        let mut magic = [0u8; 2];
        match file.read_exact(&mut magic) {
            ...
            Err(_) => Err(VolumeError::SuperblockUnreadable { device_path: path.display().to_string() }),
        }
    }
...
impl VolumeBackend for LocalFileVolumes {
    ...
    async fn attach(&self, volume_id: &VolumeId) -> Result<AttachedVolume, VolumeError> {
        let size_bytes = Self::size_of(&self.path_for(volume_id))
            .ok_or_else(|| VolumeError::NotHere { volume_id: volume_id.clone() })?;
    ...
    async fn teardown(&self, volume_id: &VolumeId) -> Result<(), VolumeError> {
        ...
        match std::fs::remove_file(&path) {
            ...
            Err(error) => Err(VolumeError::Unusable(format!("{} could not be removed: {error}", path.display()))),
        }
    }
    ...
    async fn checkpoint(&self, _volume_id: &VolumeId) -> Result<String, VolumeError> {
        Err(VolumeError::Unusable("a volume kept as a local file cannot be checkpointed".into()))
    }
...
mod tests {
    ...
    async fn a_volume_is_formatted_once_and_never_again() {
        ...
        assert!(commands.calls()[0].command.contains(&FILESYSTEM_LABEL.to_string()));

        volumes.provision(&desired_volume(|_| {})).await.unwrap();
        assert_eq!(commands.executables().len(), 1, "a formatted volume is not formatted again");
    }
    ...
    async fn a_volume_grows_and_is_never_shrunk() {
        ...
        volumes.provision(&desired_volume(|_| {})).await.unwrap();
        let grown = volumes
            .provision(&desired_volume(|volume| volume.size_bytes = VOLUME_SIZE_BYTES * 2))
            .await
            .unwrap();
        ...
        assert_eq!(
            refused,
            VolumeError::ShrinkRefused { current: VOLUME_SIZE_BYTES * 2, requested: VOLUME_SIZE_BYTES }
        );
    }
    ...
    async fn a_size_that_is_not_a_whole_sector_is_rounded_up() {
        let directory = tempfile::tempdir().unwrap();
        let volumes = backend(directory.path(), RecordingCommandRunner::succeeding());
        let attached = volumes.provision(&desired_volume(|volume| volume.size_bytes = 1000)).await.unwrap();
    ...
    async fn what_the_host_holds_is_observed_from_the_disk_rather_than_remembered() {
        ...
        volumes.teardown(&volume_id()).await.unwrap();
        assert!(matches!(volumes.attach(&volume_id()).await, Err(VolumeError::NotHere { .. })));
    }
