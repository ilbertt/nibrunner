...
use crate::services::backoff::{is_ready_to_retry, next_attempt_window, BackoffPolicy, NO_START_ATTEMPTS};
use crate::services::health::{
    apply_probe, describe_instance_failure, evaluate_instance_state, initial_tracker, next_probe_delay_ms,
    LifecycleInputs,
};
use crate::services::reconcile::plan::{InstancePlan, ReconcilePlan};
...
async fn verdict(
    ...
    state: InstanceState,
    status: &VmStatus,
    health: &crate::services::health::HealthTracker,
...
async fn settle(
    ...
    let health = if status.active && due {
        let healthy = crate::services::health::probe::probe_instance(
            &record.guest_ipv4,
            record.http_port,
            &record.health_check,
        )
    ...
    fn one_image_is_fetched_per_digest_however_many_apps_deploy_it() {
        ...
        let plan = ReconcilePlan {
            instances: vec![
                ...
                InstancePlan::Stop {
                    app_id: app_id(),
                    reason: crate::services::reconcile::InstanceStopReason::Idle,
                },
                InstancePlan::Sleep {
                    desired: desired_instance(|_| {}),
                },
